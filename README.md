...
## 0604
지난 1주 잘 쉬었나요? 일주/현제님과 같이 논문 2편이 완성이 되었고 저번에 오픈소슬 공개한 SAIDA RL 링크를 드렸는데요.
여러분들도 이 환경에서 해보고 싶을 분들 위해 윤일주님이 SAIDA RL 논문 1편 소개와 설치에 대한 가이드를 해줄 예정입니다.
그리고 시간이 허락한다면 A3C 논문을 마저 정리하고 코드 구현 들어갑니다.  오늘은 일찍 공지를 하네요^^
투표 내일 12시까지 해주시고요, 참고로 논문 아직 리뷰상태이지만 공유합니다. 출력해오시면 더 좋을 듯 합니다.

책 저자분들은 DQN과 A3C 제가 공유한 PDF처럼 다 공유해주시길 바랍니다.
